...
import BottomSheet from "@gorhom/bottom-sheet"; // Add this import
...
const SoundMusicPlayer = () => {
  ...
  const handleSave = () => {
    // Implement your save logic here, like saving the playlist
  ...
  const player = useAudioPlayer(require("@/assets/audio/naseed.mp3")); // Replace with your actual audio file
  ...
  const [feelings, setFeelings] = useState(false); // Add this state
...
const renderItemExperience = (
  { item, index }: { item: (typeof experience)[0]; index: number }
) => (
  <View style={{ 
    flexDirection: 'row', 
    alignItems: 'center', 
    ...
    {/* Playlist Button (Right Side - Only for last item) */}
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (player && !isSliding) {
        ...
        // Check if music has ended - when currentTime reaches duration and player is not playing
        if (
          player.duration > 0 &&
          player.currentTime >= player.duration - 0.5 && // Small buffer for precision
          !player.playing
        ) {
            player.seekTo(0);
          // setFeelings(true);
        ...
        >
          <View style={styles.contentContainer}>
            ...
            <View style={styles.audioControlsContainer}>
              ...
              >
                <Image
                  source={player.playing ? appImages.pause : appImages.play}
                  style={[
                    styles.playIcon,
                    { marginLeft: player.playing ? wp(0) : wp(1.2) },
                  ]}
                />
              </TouchableOpacity>
...
export default SoundMusicPlayer;
